...
class Company extends CI_Controller {
	...
	function companymanagement()
	{
		...
		$showFlag = ( $this->locale == 'hk' ) ? 'true' : 'false';
	...
	{	
		...
		if($this->form_validation->run() == FALSE)
		{
			...
			$language			= form_dropdown ('language', array('en'=>'English', 'ch'=>'Chinese'), '', 'id="selectError4" data-rel="chosen"');
		...
		if($company_info->rc > 0)
		{
			if($this->input->post('editnow',TRUE) == 'editnow')
			{
				...
				if ( $language == 'en' ){
					...
					$transExist = $this->company_model->translationExist($this->input->post('company_id'),$language);
					
					if ( $transExist->isExist == 1 ){
						// $edit_data = array(
							// 'company_id' 			=> $this->input->post('company_id'),
							// 'company_name' 			=> $this->input->post('company_name'),
							// 'company_weblink'		=> $this->input->post('company_weblink'),
						...
						$insert_data = array(
							...
							'company_description'	=> $this->input->post('company_description'),
							'language' 				=> $this->input->post('language'),
							'company_alias'			=> $this->input->post('parentId'),
