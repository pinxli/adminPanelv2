...
class Leads extends CI_Controller {
	...
	function leadstat()
	{
		...
		$startDate = '';
		$endDate   = '';
		if ( $this->input->post('searchbtn') == 'searchbtn' ){
			$startDate = $this->input->post('startDate');
			$endDate   = $this->input->post('endDate');

		}
		
		$res = $this->leads_model->leadsstatList($startDate,$endDate);
	
		//check if list is pulled. if not, list is set to empty array
		$leadsstatList = ( $res->rc > 0 ) ? $res->data->leadstat : array();
		
		$data['mainContent'] = 'leadstat_view.tpl';
